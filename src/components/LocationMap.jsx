    ...
    <section id="location" style={{ backgroundColor: '#FAF6F1' }} className="py-14 sm:py-16 scroll-mt-24">
      <div className="max-w-[1480px] mx-auto px-4 lg:px-8">
        ...
        >
          ...
          <div
            className="flex-1 w-full max-w-[680px] flex items-center justify-center relative cursor-pointer"
            onMouseEnter={() => setIsMapHovered(true)}
            onMouseLeave={() => setIsMapHovered(false)}
            onClick={() => window.open('https://maps.app.goo.gl/jNRpAnTTi5yhA3KH9?g_st=ic', '_blank')}
          >
            <motion.div
            ...
            >
              {/* Map Illustration (using video entirely to prevent duplicate maps overlapping) */}
              <div className="relative w-full max-h-[600px] flex items-center justify-center">
                <video 
                  ...
                  muted 
                  playsInline
                  className="w-full max-h-[600px] object-contain drop-shadow-sm pointer-events-none z-0"
                />
              </div>
                ...
                return (
                  ...
                  >
                    ...
                    <span className="relative flex h-4 w-4 items-center justify-center">
                      <span className={`animate-ping absolute inline-flex h-full w-full rounded-full ${isSelected || isMapHovered ? 'bg-[#7B2D16] opacity-60' : 'opacity-0'
                        }`} />
                      <span className={`relative inline-flex rounded-full transition-all duration-300 border-2 border-white shadow-md ${isSelected
                    ...
                    <AnimatePresence>
                      {showLabel && (
                        <motion.span
                          ...
                          className={`text-[10px] font-semibold px-2 py-0.5 rounded shadow-lg whitespace-nowrap mt-1 border transition-colors ${isSelected
